...
public class HtmlExtractor {
    ...
    private void renderList(Element list, StringBuilder out, boolean ordered) {
        ...
        for (Element li : list.children()) {
            ...
            String text = collapseBlankLines(inner.toString()).trim();
            if (text.isEmpty()) {
                continue;
            }
            out.append(ordered ? (idx++ + ". ") : "- ");
            // 多行列表项：首行跟在标记后，续行缩进对齐
            int firstLineBreak = text.indexOf('\n');
            if (firstLineBreak < 0) {
                out.append(text).append("\n");
            } else {
                out.append(text, 0, firstLineBreak).append("\n");
                for (String line : text.substring(firstLineBreak + 1).split("\n")) {
                    out.append("   ").append(line).append("\n");
                }
            }
        }
