...
public class AuthController {
    ...
    @Autowired
    private com.zyh.archivemind.utils.JwtUtils jwtUtils;
    ...
    public ResponseEntity<?> refreshToken(@RequestBody RefreshTokenRequest request) {
        com.zyh.archivemind.utils.LogUtils.PerformanceMonitor monitor = com.zyh.archivemind.utils.LogUtils.startPerformanceMonitor("REFRESH_TOKEN");
        String username = null;
        try {
            if (request.refreshToken() == null || request.refreshToken().isEmpty()) {
                com.zyh.archivemind.utils.LogUtils.logUserOperation("anonymous", "REFRESH_TOKEN", "validation", "FAILED_EMPTY_REFRESH_TOKEN");
            ...
            if (!jwtUtils.validateRefreshToken(request.refreshToken())) {
                com.zyh.archivemind.utils.LogUtils.logUserOperation("anonymous", "REFRESH_TOKEN", "validation", "FAILED_INVALID_REFRESH_TOKEN");
            ...
            if (username == null || username.isEmpty()) {
                com.zyh.archivemind.utils.LogUtils.logUserOperation("anonymous", "REFRESH_TOKEN", "extraction", "FAILED_NO_USERNAME");
            ...
            com.zyh.archivemind.utils.LogUtils.logUserOperation(username, "REFRESH_TOKEN", "token_generation", "SUCCESS");
        ...
        } catch (com.zyh.archivemind.exception.CustomException e) {
            com.zyh.archivemind.utils.LogUtils.logBusinessError("REFRESH_TOKEN", username, "刷新token失败: %s", e, e.getMessage());
            monitor.end("刷新token失败: " + e.getMessage());
            return ResponseEntity.status(e.getStatus()).body(Map.of("code", e.getStatus().value(), "message", e.getMessage()));
        } catch (Exception e) {
            com.zyh.archivemind.utils.LogUtils.logBusinessError("REFRESH_TOKEN", username, "刷新token异常: %s", e, e.getMessage());
